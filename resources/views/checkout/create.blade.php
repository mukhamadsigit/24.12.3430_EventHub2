...
<main class="max-w-3xl mx-auto px-6 py-20">
    <div class="mb-12">
        ...
        <p class="text-slate-500 mt-2">Lengkapi data Anda untuk
            mendapatkan tiket.</p>
    </div>
    @if(session('error'))
    <div class="mb-6 p-4 bg-red-100 text-red-700 rounded-xl fontbold">
        {{ session('error') }}
    </div>
    @endif
...
shadow-sm">
            <h3 class="text-xl font-bold mb-6 border-b pb-4">Pesanan
                Anda</h3>
...
underline underline-offset-8">📦 Data Pemesan
            ...
            <form action="{{ route('checkout.store', $event->id) }}"
                ...
                <button type="submit"
                    class="w-full py-5 bg-indigo-600 text-white
rounded-2xl font-black text-xl shadow-xl shadow-indigo-200
hover:bg-indigo-700 active:scale-95 transition-all">
                    Lanjut Pembayaran
                </button>
                <p class="text-center text-xs text-slate-400">Dengan
                    menekan tombol di atas, Anda menyetujui Syarat
                    & Ketentuan kami.</p>
            </form>
        </div>
    </div>
